...
const getListMatches = async(params)=>{
    ...
    const data = await node.callAPI("assets/search", {
      ...
      if(data.length){
        let a =[];
        for(let i of data){
            i.ownerUser = await Users.findOne({_id:i.ownerUserId}).select('username email').exec();
            i.requester = await Users.findOne({_id:i.requester}).select('username email').exec();
            a.push(i);
        }
        return a;
      }else{
